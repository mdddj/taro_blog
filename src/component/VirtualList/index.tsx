...
import { VirtualListProps, VirtualListState } from "./virtualList"
import { throttle, isH5 } from './utils'
        ...
        <View className='zt-main-list' >
          {
            twoList?.map((item, pageIndex) => {
              return (
                <View key={pageIndex} data-index={pageIndex} className={`zt-wrap-item wrap_${pageIndex}`} ref={wrapRef} >
                  {
                    item?.length > 0 ? (
                      <Block >
                        {
